...
public extension UIImage
{
    public static func icon(from font: Fonts, code: String, imageSize: CGSize, ofSize size: CGFloat) -> UIImage
    {
        ...
        drawText!.draw(in: CGRect(x:0, y:0, width:imageSize.width, height:imageSize.height), withAttributes: [NSAttributedStringKey.font : UIFont.icon(from: font, ofSize: size), NSAttributedStringKey.paragraphStyle: paragraphStyle])
...
public extension UITextView {
    func parseIcon() {
        //let text = replace(withText: (self.text! as NSString))
        let text = Hubber.replace(withText: "")
        self.attributedText = getAttributedString(text, ofSize: self.font!.pointSize)
    }
}
...
public extension UITextField {
    func parseIcon() {
        let text = Hubber.replace(withText: (self.text! as NSString))
        self.attributedText = getAttributedString(text, ofSize: self.font!.pointSize)
    }
}
